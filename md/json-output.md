...
Fields such as `payload`, `headers`, `session`, `bindings`, `trace`, `context`, `data`, and `value` are **redacted by default**: they are removed from JSON entirely. Pass `--full` (or set `redact_sensitive: false` in config) to include them, but only do this on machines where that data is safe to expose.
