...
- **Average queries per route** â€” attributed by joining query entries to their request's flush batch
...
- Most time-consuming query patterns â€” identical SQL grouped by hash with execution counts and total time
...
Analysis always respects your time window and runs over the **most recent matching entries** up to `scan_limit` â€” so on a huge production table you still get fast, meaningful answers instead of an unbounded scan. Listing (`--limit`) is independent of analysis scope.
