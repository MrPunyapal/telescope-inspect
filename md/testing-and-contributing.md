...
The suite runs against Orchestra Testbench with an in-memory SQLite database. Tests insert **realistic Telescope rows** directly into Telescope's own migrations — no mocking of the storage layer — so the actual SQL, normalization and analysis pipelines are exercised.
